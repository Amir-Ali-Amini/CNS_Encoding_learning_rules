...
def plot(
    ...
    av_inp_current = [
        torch.sum(ngs[i][recorder_index, 0].variables["I_inp"], axis=1) / ngs[i].size
        for i in range(n)
    ]
    av_current = [
        torch.sum(ngs[i][recorder_index, 0].variables["I"], axis=1) / ngs[i].size
        for i in range(n)
    ]
    for i in range(n):
        ...
        axs[i, 1].legend(
            [
                f"Current each line represent one neurok-1, {print_sum_activities and 'pink: average'}",
            ],
            loc="lower right",
            bbox_to_anchor=(1, 1),
            handlelength=0,
