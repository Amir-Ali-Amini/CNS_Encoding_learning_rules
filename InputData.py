...
class Encode(pmt.Behavior):  # time to first spike
    ...
    def encode_poison(self, ng, d):
        ...
        for i in range(num_neurons):
            spike_times = np.random.poisson(data[i], self.time)
...
class InputMatrix(pmt.Behavior):  # time to first spike
    def initialize(self, ng):
    ...
    def forward(self, ng):
        if ng.network.iteration % ng.network.input_period == 1:
            for i in range(len(self.neuron_groups)):
                self.neuron_groups[i].X = self.neuron_groups[i].vector(
                    0, dtype=torch.float64
                )
                self.neuron_groups[i].u = self.neuron_groups[i].vector(-65)
